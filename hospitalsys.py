...
def setup_db(): 
    ...
    """) 
    # (Optional) prevent duplicate appt numbers for same doctor+date 
    cur.execute(""" 
    CREATE UNIQUE INDEX IF NOT EXISTS uniq_doctor_date_no 
...
# Map login names -> passwords (change as you like) 
DOCTOR_PASSWORDS = { 
    "dr. varun": 7001, 
    "dr. hrithik": 7002, 
    "dr. salman": 7003, 
...
def ensure_unique_appt_no(doctor_name, appt_date): 
    ...
    cur.execute("""SELECT COALESCE(MAX(appt_no), 9) FROM appointments 
                   WHERE doctor_name=%s AND appt_date=%s""", 
...
def book_appointment(): 
    ...
    # Appointment date offsets similar to original behaviour 
    days_map = { 
        "Cardiologist": 3, 
        "Rheumatologist": 5, 
        "Psychiatrist": 3, 
    ...
    # Unique appointment number for doctor+date 
